...
# (user, session, query, llm, response)
...
def _preconfigure_env_for_determinism(seed: Optional[int]) -> None:
    """
    Set environment knobs that influence determinism.
    Must be called before any CUDA tensor is created.
    """
    if seed is None:
        return
    # Used by some CUDA BLAS routines. Must be set before first CUDA context creation.
    os.environ["CUBLAS_WORKSPACE_CONFIG"] = os.environ.get("CUBLAS_WORKSPACE_CONFIG", ":16:8")
    # Python dict/hash iteration order across processes.
...
def set_global_seed(seed: int, cuda_deterministic: bool = True, num_threads: Optional[int] = None) -> None:
    ...
    Note: Some GPU ops may still be nondeterministic; use with care.
    """
    random.seed(seed)
    try:
        import numpy as np
...
def compute_pairwise_loss_and_metrics(
    ...
    """
    Pairwise preference loss with accuracy/AUC.
    - Skips ties (equal ratings).
    - Optionally skips user_id==0 pairs (as in original script).
    """
    pair_losses = []
    correct, total = 0, 0
    score_diffs, labels = [], []
...
def save_if_best(
    ...
    """
    Save model/predictor when validation metric improves.
    Returns the updated best value.
    """
...
def save_periodic(
    ...
    """
    Save a periodic checkpoint (one file per epoch multiple).
    """
    os.makedirs(os.path.dirname(ckpt_path) or ".", exist_ok=True)
    if not hasattr(model, "num_layers"):
        model.num_layers = None
...
def evaluate_jsonl(
    ...
    """
    Pairwise evaluation on a JSONL pairs file using fixed embeddings.
    """
...
def train_with_cfg(cfg: Dict[str, Any]) -> Dict[str, Any]:
    # Determinism (env first, then seeds) – must be done before any CUDA tensor is created.
    ...
    metric = str(cfg["train"].get("objective", "auc")).lower()  # "auc" | "acc"
    minimize = False
    best = float("-inf")
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Validation on JSONL (if provided)
        model.eval()
        predictor.eval()
        current_val = None
        ...
        # Logging + periodic checkpoint (rolling by epoch)
