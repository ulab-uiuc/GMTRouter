...
def add_edge(
    ...
    """Append edge (src→dst) if both indices are valid."""
...
def build_interaction_edges(
    ...
    """Add edges for a pair of interactions (session→user, {q|l|r}→session, temporal)."""
...
def build_graph_edges(
    ...
    """Convert edge lists to edge_index tensors [2, E] on device."""
...
def _as_edges_dict(graph: Union[HeteroData, EdgeIndexDict]) -> EdgeIndexDict:
    """Return {HeteroEdges: edge_index} from HeteroData or an existing dict."""
...
def build_prev_session_map(graph: Union[HeteroData, EdgeIndexDict]) -> Dict[int, int]:
    """Map current_session → previous_session using S2S1 edges."""
...
def build_visible_edges(
    ...
    """Edge_index for visible interactions; links prev sessions if also visible."""
...
def parse_jsonl_pairs(path: str) -> Iterator[Tuple[dict, dict]]:
    """Yield (entry1, entry2) pairs from a JSONL file."""
...
def build_nodes_from_pairs(pairs: Iterator[Tuple[dict, dict]]):
    """Build dicts/embeddings/ratings/edges/metadata from paired entries."""
    users: Dict[int, str] = {}
    llms: Dict[int, str] = {}
...
def _normalize_ratings(r_score: List[float], device: torch.device) -> torch.Tensor:
    """Min-max normalize to [N, 1] tensor in [0,1]."""
...
def build_config_from_jsonl(path: str):
    """Build tensors/dicts from a JSONL file (returns config, nodes, edges, dicts, ratings, meta, device)."""
    device = _default_device()
...
def build_config(path: str, ckpt_path: str = None):
    """Build from JSONL (and optionally save) or load a checkpoint."""
    device = _default_device()
    ...
    if path.endswith(JSONL_SUFFIX):
        ...
        if ckpt_path is not None:
            # If ckpt_path ends with .pt, treat it as a file path; otherwise as a dir.
...
def sample_metadata(
    ...
    """Split into visible/predict sets with per-user minimum."""
...
def build_hetero_data(nodes: NodeDict, edge_index_dict: EdgeIndexDict) -> HeteroData:
    """Create a PyG HeteroData from nodes and edges."""
    data = HeteroData()
